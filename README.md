...
The supplied real-life Asahi and cruise photograph guides the central landmarks. Asahi occupies a full corner building beside Ebisubashi on Glico's bank: six floors of gray ribbon windows, metal spandrels and projecting bands support a gold billboard cabinet that wraps onto the side elevation. Two separately rectified photo textures supply the actual Asahi artwork. Twenty-four main parcels retain the concept's varied frontage and other landmarks. `scripts/dotonbori_asahi.py` authors the Asahi building; `dotonbori_architecture.py` places the parcels and `build_dotonbori.py` assembles the district.
...
Choose **Osaka Dotonbori** in the location selector, or open `/?map=dotonbori` directly. The district contains two promenades, Ebisubashi, six additional footbridges, Japanese storefronts, modeled food signs, trees, lanterns and café seating. Thirty-six live visitors use Shibuya's shared pedestrian meshes and limb animation. Their routes remain in the central district, follow actual colliders and stair treads across its original three bridges, and include 10–26 second sightseeing stops beside the railings. Visitors avoid each other and the player, and pause with the game. Two yellow open-deck river cruisers have blue seats, a center aisle, seated passengers, thin yellow rails, cream advertising boards, an open helm, tire fenders and stern safety equipment. `scripts/dotonbori_cruises.py` authors the vessels; their longer hulls stay inside the existing movement intervals. Geography, density and material detail remain stylized.
...
Cycles bakes local AO into detailed mesh colors, 4096px AO / practical-light irradiance atlases covering the expanded paving and seven bridge decks, and a seamless 512px tangent normal map for water. The light bake uses unit-strength shop lighting with sunlight excluded. The runtime scales emission and light maps once for Day, Evening, Night and Live; `lib/game/dotonbori-lighting.json` supplies matching Blender / game presets. The canal reflects the actual district and live crowd into a 1024px HDR target at water height −1.43 m. Bridges have solid downward-facing soffits, landing slabs and stone support piers for the reflection camera. Layered moving normal samples break reflections into ripples; sun glints follow the actual light direction. The reflection adds one scene render pass; Cycles and real-time reflection/shadow results are not identical.
...
node scripts/verify_dotonbori_extension.mjs
node scripts/verify_dotonbori_water.mjs
```

Rebuild before rebaking to start from the authored material colors. `assets/bakes/dotonbori.json` records actual Cycles results; `work/renders/dotonbori-day.png`, `dotonbori-evening.png` and `dotonbori-overview.png` review the delivered scene. The renderer adds a fingerprinted snapshot from the live pedestrian simulation without saving static walkers into the map. The focused verifier exercises all twelve stair entrances, bridge crossings and descents, elevated jumps, rail contact, pause, time changes, the actual reflection camera and bake data. A five-minute crowd simulation checks surface contact, use of every stair entrance, bank crossings, and resumed sightseeing stops. `scripts/verify_game.mjs` also covers existing locations and their traffic / pedestrian simulation.

`scripts/verify_dotonbori_extension.mjs` additionally reproduces the former invisible boundary, traverses all four bank extensions, and tests all sixteen added stair entrances, crossings, descents, elevated jumps, landing-side barriers, canal rails, lamps, benches and visible endpoint barriers.

The main and extended promenades share `paving_joints()` in `scripts/build_dotonbori.py`: identical joint widths, longitudinal lines and a globally aligned two-metre row spacing continue through both ±70 seams. The former background-only cross-lines are limited to scenery beyond the playable endpoints. `dotonbori-paving-west.png` and `dotonbori-paving-east.png` review the two seams.

`scripts/dotonbori_water_geometry.py` authors the entire water footprint with one upward-facing polygon per extension strip. Mirroring an end reverses its vertex order explicitly; opposed duplicate polygons can be discarded unpredictably during Draco compression. `scripts/repair_dotonbori_water_faces.py` applies this geometry to an existing baked scene without changing its lighting. The water verifier decodes the actual exported Draco primitive and checks front-face coverage and the runtime reflection mesh at both seams, every bridge and both bends.
...
Generated scene review images live in `work/renders/`, which is ignored by Git and excluded from the web build. Only `public/renders/character.png` and `character.webp` stay in the public render folder because the character panel uses them. Render scripts create their output folders as needed.

Keep scene-building inputs in source control, including `assets/references/dotonbori-asahi-cruise.jpg` and the concept artwork used to generate sign textures. Optional Kukuru and Acchichi photo references live in the ignored `work/references/` folder; their source URLs and placement notes remain tracked in `assets/references/takoyaki-sources.md`.
...
The 1024px character and 2048px map texture masters live in `assets/bakes/` and are packed into the editable `.blend` files. Browser assets use embedded WebP textures with Draco geometry; only the two small WebP irradiance files are separate downloads, one per selected map. Surface color and local AO work in Balanced as well as High graphics. High additionally enables bloom. `work/renders/crossing-evening-cycles.png` and `crossing-night-cycles.png` are Cycles comparisons using the same starting camera and avatar position as the web game (1280 × 800).
