...
export default function Game() {
  ...
  useEffect(() => {
    ...
    import('@/lib/game/world').then(({ World }) => {
      ...
      try { game.current = new World(mount.current, setStatus); }
  ...
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      ...
      if (!panel && !paused && ['Digit1','Digit2','Digit3'].includes(e.code)) setMode((['walk','motorcycle','car'] as const)[Number(e.code.slice(-1))-1]);
    };
  ...
  const switchMap = (id: MapId) => { if (id !== status.map) game.current?.switchMap(id); };
  ...
  const fullScreen = () => { if (document.fullscreenElement) document.exitFullscreen(); else document.documentElement.requestFullscreen?.().catch(() => {}); };
