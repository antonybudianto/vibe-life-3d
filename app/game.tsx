...
const displayStorageKey = 'shibuya-life-display';
const defaultDisplay = { miniMap: true, locationTitle: true };
let displaySnapshot = defaultDisplay, memoryOnlyDisplay = false;
function readDisplay() {
  if (!memoryOnlyDisplay) try {
    const saved = JSON.parse(localStorage.getItem(displayStorageKey) ?? 'null');
    const miniMap = saved?.miniMap !== false, locationTitle = saved?.locationTitle !== false;
    if (displaySnapshot.miniMap !== miniMap || displaySnapshot.locationTitle !== locationTitle) displaySnapshot = { miniMap, locationTitle };
  } catch { /* Use the current display if browser storage is unavailable. */ }
  return displaySnapshot;
}
function subscribeDisplay(onChange: () => void) {
  window.addEventListener('storage', onChange);
  window.addEventListener('display-preferences-change', onChange);
  return () => { window.removeEventListener('storage', onChange); window.removeEventListener('display-preferences-change', onChange); };
}
...
export default function Game() {
  ...
  const display = useSyncExternalStore(subscribeDisplay, readDisplay, () => defaultDisplay);
  const changeDisplay = (key: keyof typeof defaultDisplay, checked: boolean) => {
    displaySnapshot = { ...display, [key]: checked };
    try { localStorage.setItem(displayStorageKey, JSON.stringify(displaySnapshot)); } catch { memoryOnlyDisplay = true; }
    window.dispatchEvent(new Event('display-preferences-change'));
  };
  ...
  return <main className="game-shell">
    ...
    {display.locationTitle && <aside className="place-note"><span className="tiny-line"/><span>{status.map === 'dotonbori' ? 'OSAKA · 道頓堀' : status.map === 'crossing' ? 'THE CITY IS YOURS' : 'A MOMENT TO SLOW DOWN'}</span><h1>{status.map === 'dotonbori' ? <>Same river.<br/><em>New stories.</em></> : status.map === 'crossing' ? <>Same streets.<br/><em>New stories.</em></> : <>Take the<br/><em>scenic route.</em></>}</h1><p>{status.map === 'dotonbori' ? 'Follow the lanterns. Cross Ebisubashi.' : status.map === 'crossing' ? 'Find your own way through Shibuya.' : 'A little green in the heart of Tokyo.'}</p></aside>}
    ...
    <footer className="game-footer">
      <div className="map-and-name">{display.miniMap && <div className="minimap glass" aria-label={`Map: player at ${status.x.toFixed(0)}, ${status.z.toFixed(0)}`}>
      ...
      </ToggleGroup><div className="dock-divider"/><button className={`run-button ${running ? 'active' : ''}`} onClick={sprint} disabled={travel !== 'walk'} aria-pressed={running} aria-label={running ? 'Stop running' : 'Run'} title={running ? 'Stop running · Shift' : 'Run · Shift'}><Zap size={17}/><span>Run</span></button></div><div className="desktop-help"><span><kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd> Move</span><span><kbd>SPACE</kbd> Jump</span><span><kbd>R</kbd> Recenter</span></div></div>
    ...
    <Dialog open={panel !== null} onOpenChange={(v) => { if (!v) setPanel(null); }}><DialogContent className={`game-dialog ${panel === 'character' ? 'character-dialog' : ''}`}>
      ...
      {panel === 'settings' && <div className="settings-content">
        <fieldset className="display-settings"><legend className="settings-label">Display</legend>
          <label className="display-option" htmlFor="show-mini-map"><span>Show mini map</span><Switch id="show-mini-map" checked={display.miniMap} onCheckedChange={(checked) => changeDisplay('miniMap', checked)}/></label>
          <label className="display-option" htmlFor="show-location-title"><span>Show location title<small>The “Same river. New stories.” text.</small></span><Switch id="show-location-title" checked={display.locationTitle} onCheckedChange={(checked) => changeDisplay('locationTitle', checked)}/></label>
        </fieldset>
