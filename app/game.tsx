  ...
  return <main className="game-shell">
    ...
    <footer className="game-footer">
      <div className="map-and-name"><div className="minimap glass" aria-label={`Map: player at ${status.x.toFixed(0)}, ${status.z.toFixed(0)}`}>
        <MiniMap map={status.map} x={status.x} z={status.z}/><span className="north">N</span><span className="map-coordinate">{status.map === 'dotonbori' ? 'DOTONBORI · OSAKA' : status.map === 'crossing' ? '35.6595° N · 139.7005° E' : 'YOYOGI · GARDEN'}</span></div>
