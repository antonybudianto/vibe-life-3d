...
export class CitySky {
  ...
  async load() {
    const texture = await new THREE.TextureLoader().loadAsync('/textures/clouds.webp');
  ...
  dispose() { this.alive = false; this.texture.dispose(); this.mesh.geometry.dispose(); this.material.dispose(); }
}
